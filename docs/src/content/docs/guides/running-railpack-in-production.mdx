...
highly recommended to use the frontend in production**. The CLI is still used to
analyze the code and generate a build plan. The CLI simply creates a BuildKit
client and pipes the result into `docker load`. It is not something designed or
optimized to be used with high throughput. The rest of this guide will assume
you are using a custom frontend.
