...
// 	// Test with empty start in config2
// 	config2Empty := `{
// 		"providers": ["node"],
// 		"baseImage": "ubuntu:22.04",
// 		"packages": {
...
// 	var config2WithEmptyStart, expectedEmpty Config
// 	require.NoError(t, json.Unmarshal([]byte(config2Empty), &config2WithEmptyStart))
// 	require.NoError(t, json.Unmarshal([]byte(expectedEmptyJSON), &expectedEmpty))

// 	result = Merge(&config1, &config2WithEmptyStart)
// 	if diff := cmp.Diff(expectedEmpty, *result); diff != "" {
// 		t.Errorf("configs with empty start mismatch (-want +got):\n%s", diff)
// 	}
// }
